import type { Task, WeightedTask, FixedEvent, ScheduleResult, WorkingHours, TimeSlot, ScheduleAssignment } from "./types";
...
function isWeekday(date: Date) { return date.getDay() % 6 !== 0; }
...
export function scheduleTasks(
    ...
    tasks.forEach((task) => {
        ...
        let urgency = task.computedPriority * deadlineProximityFactor * (1 + task.pushCount * PUSH_MULTIPLIER)
    ...
    fixedEvents.forEach((fixedEvent) => busySlots.push({ start: fixedEvent.start, end: fixedEvent.end }))
    ...
    weightedTasks.forEach((task) => {
        const overdue = differenceInDays(task.task.dueDate ?? horizonDate, opts.today) < 0
        ...
        if (overdue || !task.task.dueDate) {
            ...
            numberOfDaysAvailable = differenceInDays(new Date(Math.min(horizonDate.getTime(), task.task.dueDate.getTime())), opts.today);
        }
        ...
        for (let i = 0; i < numberOfDaysAvailable; i++) {
            ...
            let dayBoundary = { start: day, end: day }

            let currentTime = formatTime(day);

            let startTime = (weekday ? opts.workingHours.weekDayStart : opts.workingHours.weekendStart);

            if (i > 0 || currentTime < startTime) { // so day is not today (otherwise leave start time as is as events can't be scheduled for before right now)
                const startDay = new Date(day);

                setTime(startDay, weekday ? opts.workingHours.weekDayStart : opts.workingHours.weekendStart);
            ...
            setTime(endDay, weekday ? opts.workingHours.weekDayEnd : opts.workingHours.weekendEnd);
            ...
            relevantBusySlots.forEach((slot) => {
                if (mergedSlots.length == 0) {
                    ...
                    if (slot.start <= mergedSlots[lastIndex].end) {
                        mergedSlots[lastIndex].end.setTime(Math.max(mergedSlots[lastIndex].end.getTime(), slot.end.getTime()));
                    } else {
                        mergedSlots.push({ start: new Date(slot.start), end: new Date(slot.end) });
                    }
                }
            ...
            mergedSlots.forEach((slot) => {
                ...
                cursor.setTime(Math.max(cursor.getTime(), slot.end.getTime()));
            });

            if (cursor < dayBoundary.end) {
                freeSlots.push({ start: new Date(cursor), end: dayBoundary.end });
            }
            ...
            if (!task.task.splittable) {
                for (let i = 0; i < freeSlots.length; i++) {
                    ...
                    if (slotDuration >= task.task.duration) {
                        const end = new Date(slot.start.getTime() + task.task.duration * 60 * 1000);
                        const assignment: ScheduleAssignment = { taskId: task.task.id, start: slot.start, end: end }
                ...
                for (let i = 0; i < freeSlots.length; i++) {
                    ...
                    if (slotDuration <= remaining) {
                        end.setTime(slot.end.getTime());
                    } else {
                        end.setTime(slot.start.getTime() + remaining * 60 * 1000);
                    }

                    candidateSlots.push({ start: new Date(slot.start), end: end });

                    remaining -= differenceInMinutes(end, slot.start);
                ...
                if (assigned) {
                    candidateSlots.forEach((slot) => {
                        const assignment: ScheduleAssignment = { taskId: task.task.id, start: slot.start, end: slot.end };
                        result.assignments.push(assignment);
